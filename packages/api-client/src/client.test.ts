...
describe("GatewayClient", () => {
  it("normalizes an http(s) origin and rejects path-bearing URLs", () => {
    expect(normalizeGatewayOrigin("https://gateway.example.com/")).toBe(
      "https://gateway.example.com",
    );
    expect(() => normalizeGatewayOrigin("ftp://gateway.example.com")).toThrow();
    expect(() =>
      normalizeGatewayOrigin("https://gateway.example.com/v1"),
    ).toThrow();
  });
  ...
  it.each([
    ...
    { data: [{ nope: true }] },
  ])("rejects invalid model cardinality or shape", async ({ data }) => {
    const client = new GatewayClient("https://gateway.example.com", {
      fetch: async () => Response.json({ object: "list", data }),
    });
    await expect(client.discoverModel()).rejects.toMatchObject({
  ...
  it("maps a safe OpenAI error and retains the exposed request id", async () => {
    ...
    expect(error).toMatchObject({
      status: 429,
      code: "rate_limit_exceeded",
      requestId: "13df274a-2e13-421d-8ec7-e77a528aaa6b",
    });
  });
  ...
  it("never exposes a malformed error body", async () => {
    const client = new GatewayClient("https://gateway.example.com", {
      fetch: async () =>
        new Response("provider=https://secret.example model=private", {
          status: 502,
        }),
    });
    await expect(client.discoverModel()).rejects.toMatchObject({
      message: "پاسخ امن و معتبری از درگاه دریافت نشد.",
      code: "invalid_response",
    });
  });
    ...
    const fetcher = vi.fn(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        expect(new Headers(init?.headers).get("authorization")).toBe(
          "Bearer anonymous-placeholder",
        );
        return Response.json({ object: "list", data: [model] });
      },
    );
    await new GatewayClient("https://gateway.example.com", {
      fetch: fetcher,
      authorizationPlaceholder: "anonymous-placeholder",
    }).discoverModel();
  });
});
