...
const frame = (content: string) =>
  `data: ${JSON.stringify({ id: "chatcmpl-1", object: "chat.completion.chunk", created: 1, model: "liara-assistant", choices: [{ index: 0, delta: { content }, finish_reason: null }] })}\n\n`;
const terminalFrame = `data: ${JSON.stringify({ id: "chatcmpl-1", object: "chat.completion.chunk", created: 1, model: "liara-assistant", choices: [], x_liara: { schema_version: 1, documentation_revision: "a".repeat(64), intent: { kind: "direct", missing_fields: [], topic_changed: false }, answer_path: "generated", citations: [], next_steps: [], reuse: "none" } })}\n\n`;

function streamResponse(
  parts = [frame("س"), frame("لام"), "data: [DONE]\n\n"],
  delay = 0,
) {
  const framed = parts.flatMap((part) =>
    part.includes("data: [DONE]") ? [terminalFrame, part] : [part],
  );
  return new Response(
    new ReadableStream({
    ...
    }),
    {
      headers: {
        ...
        "x-request-id": "13df274a-2e13-421d-8ec7-e77a528aaa6b",
      },
    },
  );
}
...
describe("ChatPanel", () => {
  ...
  it("discovers once, renders increments, and sends follow-up context", async () => {
    ...
    await user.type(screen.getByLabelText("پیام"), "سلام");
    await user.click(screen.getByRole("button", { name: "ارسال" }));
    await screen.findByText("سلام", {
      selector: ".hackathon-chat__message--assistant p",
    });
    await user.type(screen.getByLabelText("پیام"), "ادامه");
    await user.click(screen.getByRole("button", { name: "ارسال" }));
    await waitFor(() => expect(bodies).toHaveLength(2));
    expect(
      fetcher.mock.calls.filter(([url]) => String(url).endsWith("/v1/models")),
    ).toHaveLength(1);
  ...
  it("recovers model discovery and never presents a silently inert composer", async () => {
    let discoveryAttempts = 0;
    const fetcher = vi.fn(async (input: RequestInfo | URL) => {
      if (String(input).endsWith("/v1/models")) {
        discoveryAttempts += 1;
        if (discoveryAttempts === 1) throw new TypeError("fetch failed");
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "درگاه گفتگو در دسترس نیست",
    );
    expect(screen.getByLabelText("پیام")).toBeDisabled();
    expect(screen.getByRole("button", { name: "ارسال" })).toBeDisabled();

    await user.click(
      screen.getByRole("button", { name: "تلاش برای اتصال دوباره" }),
    );
    await waitFor(() => expect(screen.getByLabelText("پیام")).toBeEnabled());
    await user.type(screen.getByLabelText("پیام"), "سلام");
    await user.click(screen.getByRole("button", { name: "ارسال" }));
  ...
  it("marks partial output stopped, keeps it in follow-up context, and retries by replacement", async () => {
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (completion === 1)
          return new Response(
            new ReadableStream({
              start(value) {
                value.enqueue(encoder.encode(frame("نیمه")));
              },
            }),
          );
        return streamResponse();
    ...
    render(<ChatPanel gatewayUrl="https://gateway.example.com" />);
    await screen.findByText("از کجا شروع کنیم؟");
    await user.type(screen.getByLabelText("پیام"), "سؤال");
    await user.click(screen.getByRole("button", { name: "ارسال" }));
    await screen.findByText("نیمه");
  ...
  it("persists same-tab state and restores streaming output as stopped", async () => {
    ...
    render(<ChatPanel gatewayUrl="https://gateway.example.com" />);
    await screen.findByText("نیمه");
    expect(await screen.findByText("پاسخ متوقف شد")).toBeVisible();
  });
  ...
  it.each([
    ...
    "shows a safe Persian recovery for HTTP %i",
  ...
  it("exposes busy state and a terminal-only live status", async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) =>
        String(input).endsWith("/v1/models")
          ? Response.json(model)
          : streamResponse([frame("الف"), frame("ب"), "data: [DONE]\n\n"], 5),
      ),
    );
    const user = userEvent.setup();
    render(<ChatPanel gatewayUrl="https://gateway.example.com" />);
  ...
  it("submits with Enter and preserves Shift+Enter", async () => {
    ...
    render(<ChatPanel gatewayUrl="https://gateway.example.com" />);
    const input = await screen.findByLabelText("پیام");
    fireEvent.change(input, { target: { value: "خط اول" } });
