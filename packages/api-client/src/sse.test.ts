...
async function collect(
  ...
  const values = [];
  for await (const value of parseOpenAIStream(input, { limits, signal }))
    values.push(value);
  return values;
}
...
describe("parseOpenAIStream", () => {
  it("parses split LF/CRLF frames and ignores comments", async () => {
    ...
    expect(values.map((value) => value.choices[0]?.delta.content)).toEqual([
      "س",
      "لام",
    ]);
  });
  ...
  it.each([
    ...
    await expect(
      collect(response(parts as string[]), limits as Partial<StreamLimits>),
  ...
  it("requires an event-stream response body", async () => {
    await expect(collect(new Response(null))).rejects.toBeInstanceOf(
      GatewayClientError,
    );
  });
});
