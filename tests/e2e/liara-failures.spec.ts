...
for (const fixture of [
  { name: "zero", body: { object: "list", data: [] } },
  {
    ...
    body: {
      object: "list",
      data: [publicModel, { ...publicModel, id: "other" }],
    },
  },
  { name: "malformed", body: { data: [publicModel] } },
])
  test(`${fixture.name} model discovery blocks completion`, async ({
    page,
  }) => {
    let completionCalls = 0;
    await installModels(page, fixture.body);
...
for (const fixture of [
  ...
  test(`sanitizes ${fixture.status} recovery`, async ({ page }) => {
    ...
    await expect(page.getByRole("alert")).toContainText(fixture.text);
    await expect(page.getByRole("alert")).toContainText("13df274a");
  });

test("reports incomplete stream without rendering raw provider data", async ({
  page,
}) => {
  await installModels(page, { object: "list", data: [publicModel] });
  await page.route("**/v1/chat/completions", (route) =>
    route.fulfill({
      ...
      body: `data: ${JSON.stringify({ id: "chatcmpl-e2e", object: "chat.completion.chunk", created: 1, model: "liara-assistant", choices: [{ index: 0, delta: { content: "private-provider.example" }, finish_reason: null }] })}\n\n`,
    }),
  );
  await page.goto("/chat");
  await page.getByLabel("پیام").fill("سلام");
