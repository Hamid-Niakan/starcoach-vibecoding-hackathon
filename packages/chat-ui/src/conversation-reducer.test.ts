...
describe("conversationReducer", () => {
  it("creates ordered user/assistant pairs with one active stream", () => {
    ...
    expect(
      state.messages.map(({ role, turnId, status }) => ({
        role,
        turnId,
        status,
      })),
  ...
  it("appends deltas and completes, stops, or fails the assistant", () => {
    const start = conversationReducer(createInitialConversationState(), {
      type: "turn.started",
      turnId: ids[0]!,
      userId: ids[1]!,
      assistantId: ids[2]!,
    ...
    const delta = conversationReducer(start, {
      type: "assistant.delta",
      turnId: ids[0]!,
      content: "پاسخ",
    });
    expect(
      conversationReducer(delta, {
        type: "assistant.stopped",
        turnId: ids[0]!,
        requestId: ids[3],
      }).messages[1],
    ).toMatchObject({ content: "پاسخ", status: "stopped", requestId: ids[3] });
    expect(
      conversationReducer(delta, {
    ...
    ).toMatchObject({ status: "completed", metadata });
    expect(
      conversationReducer(delta, {
  ...
  it("includes stopped results and excludes failed/current streams from visible context", () => {
    ...
    expect(projectVisibleContext(state.messages)).toEqual([
      ...
      { role: "assistant", content: "نیمه" },
      { role: "user", content: "دو" },
    ]);
  });
  ...
  it("retries by replacing the old assistant without duplicating its user", () => {
    ...
    const retried = conversationReducer(state, {
      type: "turn.retried",
      turnId: ids[0]!,
      assistantId: ids[3]!,
